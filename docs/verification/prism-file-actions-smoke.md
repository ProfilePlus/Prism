...
> 当前状态：已补系统废纸篓 Tauri command、macOS Finder trash 超时保护和永久删除 fallback 自动化回归；macOS Finder AppleScript 路径已用同脚本 smoke 通过；Prism `.app` 已真实打开临时 workspace 并截图，文件树删除点击尚未执行；Windows 桌面 smoke 尚未执行。
...
- `src-tauri/src/lib.rs`：新增 `move_path_to_trash` Tauri command。macOS 使用可超时终止的 `osascript` 子进程调用 Finder 废纸篓；非 macOS 继续使用 Rust `trash` crate。
...
- `src-tauri/src/lib.rs` 单元测试覆盖 `move_path_to_trash` 对不存在路径的拒绝，避免无效路径进入系统废纸篓调用；同时覆盖系统废纸篓子进程正常返回和超时 kill 分支。
...
这些测试证明删除 contract 和 Rust command 编译可用；macOS Finder AppleScript 已补同脚本 smoke，但还不能替代真实 Prism UI 文件树删除流程和 Windows Explorer 桌面验证。
...
- 删除点击尚未执行：通过 GUI 删除本地文件需要用户在动作前确认，且当前只验证了“真实 App 打开临时 workspace 和文件树可见”。
...
真实 Prism UI 文件树删除点击和 Windows 桌面 smoke 尚未执行；当前具备自动化 contract、Rust 编译、macOS Finder AppleScript 同路径 smoke、真实 `.app` 打开文件树截图和手动 smoke 协议。
