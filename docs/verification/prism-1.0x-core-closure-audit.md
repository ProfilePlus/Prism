...
- UI 行为中等：React 组件测试覆盖面板、状态栏和 CodeMirror 命令事件接线；全局编辑器事件已覆盖表格、模板、行内格式、heading、任务列表和异常 payload 防护；列表 keymap 已覆盖 Enter / Tab / Shift+Tab 的 CodeMirror 按键路径；Markdown 链接补全和 `[[note]]` 内链补全已覆盖挂载后 CodeMirror 上下文；图片拖拽 / 粘贴已有 CodeMirror DOM 事件接线回归和 helper 层 fs 写入 contract，但没有真实系统剪贴板 / Finder / Explorer 文件写入 smoke。

### 缺口

- 图片粘贴已有 macOS 真实 `.app` 系统剪贴板 smoke：PNG 写入 `NSPasteboard` 后通过原生 `Edit > Paste` 触发，产物写入 `assets/index/image-20260515-141341.png`，正文插入相对 Markdown 图片路径。图片拖拽仍只有 helper 与 CodeMirror DOM 接线自动化，还没有 macOS Finder 拖入、Windows Explorer 拖入和 Alt/Option 修饰键差异验证。
...
`docs/verification/prism-writing-efficiency-smoke.md` 已覆盖图片粘贴、图片拖拽、Alt/Option 原路径、链接补全、链接诊断、表格命令、列表续写和模板插入；下一步应真实执行桌面 smoke，把组件级证据提升为用户可见工作流证据。
...
- `src-tauri/src/lib.rs` 暴露 `move_path_to_trash`；macOS 路径使用可超时终止的 `osascript` 子进程调用 Finder 废纸篓，非 macOS 继续使用 Rust `trash = 4.1.1`。
...
- `docs/verification/prism-file-actions-smoke.md` 记录文件动作真实桌面 smoke 协议。

### 验证强度

- 自动测试中等偏强：新增 contract 测试能阻止菜单 action 漂移回到未知文件动作；删除流程已有首次取消、废纸篓优先和永久删除 fallback 回归；工作区聚焦刷新已有 hook 级回归。
- Rust 编译中等：`cargo test` 和 `cargo check` 已覆盖新增 Tauri command 的编译链路；`move_path_to_trash` 已覆盖不存在路径拒绝，系统废纸篓子进程正常返回和超时 kill 分支也有单元测试。
- 系统废纸篓自动化 smoke 受阻：直接运行旧 macOS `trash::delete` smoke 会卡在 Finder `osascript ... delete` 超过 60 秒，已记录在 `docs/verification/prism-file-actions-smoke.md`；当前实现已把 Finder 不响应降级为 8 秒超时错误，让前端进入永久删除二次确认 fallback。
- 真实桌面行为仍需要后续 smoke 覆盖：系统废纸篓、重命名、新窗口打开、复制路径和打开位置在真实 macOS / Windows 文件管理器中的行为仍需人工确认。

### 缺口

- 系统废纸篓已有代码路径、自动化 contract、无效路径校验和 macOS Finder 超时保护，但仍尚未真实验证 Finder / Explorer 废纸篓结果。
- 文件动作真实桌面 smoke 还未执行，尤其是 Windows Explorer 的打开位置和路径复制。
